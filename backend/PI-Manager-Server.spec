...
datas = [
    ...
    # 将前端构建产物（dist）静态打包进可执行文件的 frontend_dist 目录中
    (os.path.join(backend_dir, '../frontend/dist'), 'frontend_dist'),
]

# 手动补全 PyQt5、QWebChannel 及加密库等运行时动态加载的隐藏导入
hiddenimports += [
    'PyQt5',
    'PyQt5.QtCore',
    'PyQt5.QtGui',
...
a = Analysis(
    ['run_qt.py'],  # 切换入口文件为 Qt 客户端启动器 run_qt.py
    pathex=[backend_dir],
    binaries=[],
    datas=datas,
